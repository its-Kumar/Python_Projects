...
class GameScreen(Tk):
    def __init__(self):
        ...
        self.heading = Label(
            self.tframe,
            text='Rock Paper Scissor by Ayush Solanki',
            font=Font(
                size=25),
            bg=self.bgcolor)
        self.heading.pack()
    ...
    def button_panel(self):
        ...
        self.left_rock = Button(
            ...
            bg='#61cf7b',
            activebackground='#f2ce55',
            command=self.selectrock)
        self.left_rock.pack(pady=5)
        self.left_paper = Button(
            self.lframe,
            text='Paper',
            width=15,
            font=Font(
            ...
            bg='#61cf7b',
            activebackground='#f2ce55',
            command=self.selectpaper)
        self.left_paper.pack(pady=5)
        self.left_scissor = Button(
            self.lframe,
            text='Scissor',
            width=15,
            font=Font(
    ...
    def game_panel(self):
        ...
        self.user_label = Label(self.lblframe, text='User: 0', font=Font(
            size=25,), bg=self.bgcolor, fg='#ffffff')
        self.user_label.pack(side=LEFT, padx=(25, 100))
        self.computer_label = Label(
            self.lblframe, text='Computer: 0', font=Font(
                size=25), bg=self.bgcolor, fg='#ffffff')
        self.computer_label.pack(side=RIGHT, padx=(100, 25))
        self.user_canvas = Canvas(
            self.rframe,
            width=200,
            height=200,
            highlightthickness=0,
        ...
        if self.user_points < 5 and self.comp_points < 5:
            ...
            if self.user_points >= 5:
                res = messagebox.askyesno(
                    "User Won!!", "User wins the game :) want to play again?")
                if res:
                    self.resetgame()
            elif self.comp_points >= 5:
                res = messagebox.askyesno(
                    "Computer Won!!",
                    "Computer wins the game :( want to play again?")
                if res:
                    self.resetgame()
        else:
            pass
