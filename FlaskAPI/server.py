...
def get_all_users_descending():
    ...
    al_users_ll = linked_list.LinkedList()

    for user in users:
        al_users_ll.insert_beginning(
            {
                "id": user.id,
                "name": user.name,
                "email": user.email,
    ...
    return jsonify(al_users_ll.to_list()), 200
...
def get_one_user(user_id):
    ...
    user = al_users_ll.get_user_by_id(user_id)
...
def get_numeric_post_bodies():
    ...
    for _ in range(len(blog_posts)):
        ...
        for char in post.data.body:
            numberic_body += ord(char)

        post.data.body = numberic_body
        return_list.append(
            {
                "id": post.data.id,
